...
# select only the numeric variables to make a correlation plot
num_vars <- sapply(spito_new, is.numeric)
df_numeric <- spito_new[, num_vars]
# calculate the correlation matrix
cor_matrix <- cor(df_numeric)
# create a corplot with the correlation matrix
corrplot(cor_matrix, method = "circle")
#obviously we need to remove price_per_sq_meter from our dataset.id and agent_id which
#are not related with the price.Then we will model our data using price as response
#variable and all the other variables as independent to identify the most important attributes
#in predicting the price of a property 
a3_data<-subset(spito_new, select = -c(id, agent_id, price_per_sq_meter))
m_price<-lm(a3_data$price~., data=a3_data)
summary(m_price)
# Perform both forward and backward elimination
model <- step(m_price, data = a3_data, direction = "forward", trace = 0)
# View the selected predictors
summary(model)
#For the backward elimination to perform we need to remove nas from the dataset
# Load the "MASS" package
library(MASS)
# Remove missing values from the dataset
a3_data_na <- na.omit(a3_data)
# Run the backward elimination regression
m_price1 <- lm(a3_data_na$price ~ ., data = a3_data_na)
model1 <- step(m_price1, data = a3_data, direction = "backward", trace = 0)
summary(model1)
#Step function offers us the ability to check all the possible models starting from
#the one with all the variables (52) and removing those not needed (12). We can see the
#significant variables for the model (p-value<0.05) and those that estimate is high,
#meaning how much the price will change by changing the independent variable by 1.The model
#we used the backward elimination has the higher Adjusted R-squared:  0.9716 and is the one we keep.
#This model show us the most important attributes to the price of any property
#and the variables that an agent should use to valuate each property.

#If I had more time I would try to make better plots and better presentation for all the
#assignments. Other than that especially in 3rd assignment the model should be evaluated more
#in terms of its predictive ability, by splitting the dataset into train and test and 
#performing the predictions to the test dataset from the model found from the train.
